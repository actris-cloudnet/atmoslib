...
def test_surface_value_returned_at_first_level():
    ...
    p = hydrostatic_pressure(t, q, z, p_sfc)
...
def test_moist_atmosphere_lighter_than_dry():
    ...
    p_sfc = np.array(101325.0)
    p_dry = hydrostatic_pressure(t, np.zeros(4), z, p_sfc)
    p_moist = hydrostatic_pressure(t, np.full(4, 0.015), z, p_sfc)
...
def test_2d_profile_matches_per_row_1d():
    ...
    p_2d = hydrostatic_pressure(t, q, z, p_sfc)
    assert p_2d.shape == t.shape

    for i in range(n_time):
        p_1d = hydrostatic_pressure(t[i], q[i], z, p_sfc[i])
