...
def test_liquid_water_specific_attenuation():
    T = np.array([273.15])
...
def test_gas_specific_attenuation():
    ...
    # propagation prediction methods, version 8.3.0
    # www.itu.int/en/ITU-R/study-groups/rsg3/rwp3m/Pages/digprod.aspx
...
def test_rain_specific_attenuation():
    # P.838-3 Sp.Att in Validation examples for Study Group 3 Earth-space
    # propagation prediction methods, version 8.3.0
    # www.itu.int/en/ITU-R/study-groups/rsg3/rwp3m/Pages/digprod.aspx
    theta = [
        31.07699124,
    ...
    for theta0, f0, R0, tau0, gamma0 in zip(theta, f, R, tau, gamma, strict=True):
        gamma1 = rain_specific_attenuation(
            R0, f0, "horizontal" if tau0 == 0 else "vertical", theta0
        )
        assert_allclose(gamma1, gamma0)
